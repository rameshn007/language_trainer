...
| **Phase 2** | **Steering Wheel & Native Media Controls** | 🔄 **In Progress** | Fix 5-source skip glitch so physical steering wheel buttons, Control Center, and Lock Screen skip full words instead of silence chunks. | `AppDelegate.swift`, `carplay_service.dart`, `listen_repeat_view_model.dart` |
